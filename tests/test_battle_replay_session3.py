"""Integration test: replay battle_session_3 (2026-05-16 20:12 ~ 20:20 PvP) through the full pipeline."""
...
SESSION_DIR = Path(__file__).resolve().parent / "fixtures" / "packets" / "battle_session_3"
...
class TestPacketLoading:
    def test_packets_loaded(self, battle_packets):
        assert len(battle_packets) > 0, "No battle packets loaded from session 3"
...
class TestBattleStateReplaySession3:
    ...
    def test_my_pets_populated(self, replay_result):
        _, state = replay_result
        assert len(state["my_pets"]) == 6, f"Expected 6 my_pets, got {len(state['my_pets'])}"

    def test_opp_pets_populated(self, replay_result):
        _, state = replay_result
        assert len(state["opp_pets"]) == 4, f"Expected 4 opp_pets, got {len(state['opp_pets'])}"

    def test_battle_result(self, replay_result):
        _, state = replay_result
        assert state["result"] == "WIN_HP", f"Expected WIN_HP, got {state['result']}"

    def test_rounds_tracked(self, replay_result):
        _, state = replay_result
        assert state["round"] == 15, f"Expected 15 rounds, got {state['round']}"

    def test_events_collected(self, replay_result):
        _, state = replay_result
        assert len(state["events"]) >= 100, f"Too few events: {len(state['events'])}"
    ...
    def test_phase_finished(self, replay_result):
        _, state = replay_result
        assert state["phase"] == "finished", f"Expected phase=finished, got {state['phase']}"


# ---------------------------------------------------------------------------
# TestEventFormatterReplaySession3
# ---------------------------------------------------------------------------
...
class TestEventFormatterSession3:
...
# ---------------------------------------------------------------------------
# TestBattleSummarySession3
# ---------------------------------------------------------------------------


class TestBattleSummarySession3:
    ...
    def test_summary_pet_counts(self, replay_result):
        _, state = replay_result
        summary = compute_battle_summary(state)
        assert len(summary["my_pets_final"]) == 6
        assert len(summary["opp_pets_final"]) == 4
...
# ---------------------------------------------------------------------------
# TestBattleReportGenerationSession3 — generate a readable report
# ---------------------------------------------------------------------------


class TestBattleReportSession3:
    ...
    def test_report_saved(self, replay_result, tmp_path):
        from scripts.generate_battle_report import generate_report
        report = generate_report(SESSION_DIR)
        out = tmp_path / "battle_session_3_report.txt"
