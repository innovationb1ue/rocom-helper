...
class RocoBinDecoder:
    def __init__(self, data: bytes, schema: Optional[dict], bin_type: str,
                 loc_decoder: Optional['RocoBinDecoder'] = None):
    ...
    def _read_array(self, prop: dict, loc: Optional['RocoBinDecoder'],
    ...
    def _read_property(self, prop: dict, loc: Optional['RocoBinDecoder']) -> Any:
        t = prop["Type"]
        if t == "EUint32":
    ...
    def _parse_struct(self, schema: dict, loc: Optional['RocoBinDecoder']) -> dict:
    ...
    def _parse_nested_struct(self, schema: dict,
                             loc: Optional['RocoBinDecoder']) -> dict:
        const_idx = self.reader.read_int32()
        entry = self.constants_table[const_idx - 1]
        saved = self.reader.pos
        self.reader.seek(entry.offset)
...
def find_schema(bytes_path: str, schema_dir: Optional[str] = None) -> Optional[str]:
...
def decode_file(bytes_path: str, schema_path: Optional[str] = None,
                bin_type: Optional[str] = None, loc_path: Optional[str] = None,
                schema_dir: Optional[str] = None) -> dict:
    """解码单个 .bytes 文件"""
    if bin_type is None:
...
def batch_decode(src_dir: str, schema_dir: str, out_dir: str,
                 bin_type: Optional[str] = None, loc_dir: Optional[str] = None):
    """批量解码目录下所有 .bytes 文件"""
    src = Path(src_dir)
    out = Path(out_dir)
